...
window.addEventListener("load", () => {
  ...
  totalPopulation = document.querySelector("#totalPopulationList");
...
function render() {
  ...
  function renderSummary() {
    ...
    const totalPopulationN = allCountries.reduce((accumulator, current) => {
      return accumulator + current.population;
    }, 0);

    const totalPopulationFavoritesN = favoriteCountries.reduce(
      (accumulator, current) => {
        return accumulator + current.population;
      },
      0
    ...
    totalPopulationFavorites.textContent = formatNumber(
      totalPopulationFavoritesN
    );
    totalPopulation.textContent = formatNumber(totalPopulationN);
  }
